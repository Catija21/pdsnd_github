...
# Project Overview
Exploring data related to bike share systems for three major cities in the United States—Chicago, New York City, and Washington. We will be answering interesting questions about it by computing descriptive statistics and making visualizations!
...
#Loading some libraries we are in need to, help in wrangling the dates and easier manipulate the data.
...
#Creating function 'Common_month' that takes the short and the complete city name as input and returns the most common month and the related data visualization
...
The histogram shows clearly that the month 06 'June' holds the highest number of travels, about 100000 travels, followed by 'May' with about 66000 travels, while the smallest number about 21900 is for month 01 'January'
```{r}
common_month(ny,"New York")
```

For New York city the month number 06 'June' is also the most common with more than 75000 travels followed by month 05 'May', while the month 01 'January' holds the least number of travels about 32000.
```{r}
common_month(wash,"Washington")
```


The month number 06 'June' is the most common for Washington as well with more than 68000 travels, followed by month 04 'April' with more than 62000 travels, and the smallest number about 30000 is assigned for month 01 'January'.
...
The actual values resulting from the statistical calculations confirm exactly what we had appearing on the data visualizations above, the max and the min values confirm the ranges we had from the charts.

## Q02 - Popular stations and trip | What is the most common start station?


```{r}
#Creating new function, input: short and the complete name, returns: the most common Start Station with the data visualization, as well as the summary stats related to the top 5 stations
...
The previous bar chart compares the counts of travels of the top 5 Start stations in Chicago. 'Streeter Dr & Grand Ave' is the most common, followed by 'Clinton St & Washington Blvd'. The counts range from about 3400 to 6900.

```{r}
common_station(ny, "New York")
```

The bar chart for New York shows that 'Pershing Square North' is the most common followed by 'E 17 St & Broadway'. The number of travels range from about 1900 to 3000.
```{r}
common_station(wash, "Washington")
```

'Columbus Circle / Union Station' is the most common station followed by 'Lincoln Memorial' and the last one is '15th & P St NW'. The values range between about 5600 and 3500.
...
The median line is much higher for Customers, which indicates that Users of type 'Customer' registered longer travel time than 'Subscribers',  50% of Customers spent about 23 minutes while the Subscribers spent about 9 minutes.  
...
```
By looking at the actual values obtained we can confirm our results on the graph, the first quartile at 25% for Customers is 15.45 and looks right on the plot. The 75% mark for Subscribers is 14.983 and it is exactly what had showing on the boxplot.
...
From the plot we observe that the Interquartile Range for "Customers" lays between 14 and 29 mins, and the median is at about 22 mins, while for "Subscribers" the IQR is between 6 and 16 mins, and its median falls at about 9 mins. The difference between them is considerable that the time for Subscribers is almost half of that for Customers. 
...
And finally to check on the exactness of the data apearing on the graph, the 25% mark for Customers is 15.334 and the 75% mark for Subscribers is 15.045 mins, which is very true!
